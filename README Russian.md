...
Знак `$` префиксно дополняет название секции, позволяя искать по файлу такой шаблон `$[НАЗВАНИЕ-СЕКЦИИ]` и тем самым **поиск производится только по названиям секций**.
...
Такая структура означает, что секция написанная ниже строится и наследует правила, описанные в предыдущих секциях. Это приведёт к меньшему количеству проблем специфичности и отменяющих переопределений, а также к более качественной CSS архитектуре.

Для дальнейшего чтения по этой теме, я не могу рекомендовать вам книгу [SMACSS](http://smacss.com) от Джонатана Снука настолько, насколько она того заслуживает.
...
Наглядно видно, что `.widget-heading` является дочерним элементом `.widget`, так как `.widget-heading` имеет дополнительный отступ относительное селектора `.widget`. Эту полезную информацию разработчики могут считать просто взглянув на отступы в наших стилях.
...
гораздо больше смысла в использовании однострочных правил.
...
**Никогда не используйте классы CSS, нужные для _стилизации_, в JavaScript-логике.** Связывание логики скриптов с оформлением ведёт к тому, что мы не сможем использовать одно без другого.
...
Вы должны документировать и комментирвоать так много, насколько возможно; всё, что кажется вам прозрачным и говорящим за себя, может не быть таковым для другого разработчика. Пишите кусок кода и сразу после этого объясняйте его.
...
Если вы будете писать сначала разметку, то сможете сфокусироваться на информации, контенте и семантике, а применять соответствующие классы и стили будете _после_.
...
Они могут встречаться в вашей разметке для JS и идентификации фрагментов, но для стилизации используйте только классы. Вы не захотите видеть только один идентификатор во всех файлах стилей!
...
**Запомните:** Классы на самом деле ни семантичны, ни не семантичны; Они бывают рациональны и не! Перестаньте беспокоиться о «семантике» имён классов и выберите что-нибудь рациональное и безопасное в будущем.
...
Например, использование `.dropdown-nav li:hover ul { top: 37px; }`, чтобы расположить выпадающее меню внизу навигации не принесёт ничего хорошего, так как 37px магическое число. 37px работает только потому, что в этом конретном сценарии элемент `.dropdown-nav` оказался высотой в 37px.

Вместо этого мы должны использовать `.dropdown-nav li:hover ul { top: 100%; }`, что означает: вне зависимости какой высоты будет меню `dropdown-nav`, выпадающее всегда будет сдвинуто на 100% от верха.

Каждый раз, когда вы жестко задаёте число, подумайте дважды; если вы можете избежать этого, используя ключевые слова или синонимы (например, `top: 100%` — сдвинуть на 100% от верха) или — даже лучше — не используя никаких единиц измерений, избегайте.

Каждое установленное вами числовое значение, скорее всего, было необязательным.

## Стили в условных комментариях

Использования отдельных файлов стилей для IE, по большому счёту, можно избежать. Исключением может быть необходимость восполнить вопиющие недостатки поддержки (например, PNG с альфа–каналом в IE6).
